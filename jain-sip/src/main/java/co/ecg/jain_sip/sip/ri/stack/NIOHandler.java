...
public class NIOHandler {
    ...
    protected void removeSocket(SocketChannel channel) {
        ...
        log.debug("Trying to remove cached socketChannel without key"
        ...
        synchronized (socketTable) {
            ...
            for (String key : keys) {
                ...
                log.debug("Removing cached socketChannel without key"
    ...
    public void stop() {
        try {
            // Reworked the method for https://java.net/jira/browse/JSIP-471
                ...
                log.debug("existing socket in NIOHandler " + socketTable.keySet());
            ...
            while (entriesIterator.hasNext()) {
                ...
                NioTcpMessageChannel messageChannel = entry.getValue();
                    ...
                    log.debug("stop() : Removing socket " + messageChannel.key
    ...
    public SocketChannel createOrReuseSocket(InetAddress inetAddress, int port) throws IOException {
        ...
        try {
            ...
            if (channel != null && (!channel.isConnected() || !channel.isOpen())) {
                ...
                log.debug("Channel disconnected " + channel);
                channel = null;
            }
            if (channel == null) { // this is where the threads will race
                ...
                log.debug("create channel = " + channel + "  " + inetAddress + " " + port);
                if (channel != null && channel.isConnected()) {
                    putSocket(NIOHandler.makeKey(inetAddress, port), channel);

                    log.debug("channel cached channel = " + channel);
                }
            }
            return channel;
        } finally {
            ...
            log.debug("Returning socket " + key + " channel = " + channel);
        }
    }
}
