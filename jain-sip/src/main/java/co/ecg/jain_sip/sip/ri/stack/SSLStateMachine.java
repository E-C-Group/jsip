...
public class SSLStateMachine {
	...
	public void wrap(ByteBuffer src, ByteBuffer dst, 
		...
		synchronized (wrapLock) {
			...
			if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				log.debug("Wrapping " + src + ", buffers size " + pendingOutboundBuffers.size());
			}
			...
			loop:while(true) {
				...
				try {
					...
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Wrap result " + result + " buffers size " + pendingOutboundBuffers.size());
					}
				} finally {
					if(!currentBuffer.message.hasRemaining()) {
						pendingOutboundBuffers.remove();
						if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
							log.debug("REMOVED item from encryption queue because it has no more data, all is done, buffers size now is "
				...
				if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					log.debug("Remaining " + remaining +  " queue size is " + pendingOutboundBuffers.size());
				}
							...
							if(sslEngine.getSession() != null) {
								if(!ClientAuthType.Disabled.equals(channel.getSIPStack().getClientAuth()) && !ClientAuthType.DisabledAll.equals(channel.getSIPStack().getClientAuth())) {
									...
									try {
										...
										if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
											log.debug("sslEngine.getSession().getPeerCertificates() are not available, which is normal if running with gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE=Disabled");
										}
									}
								}
	...
	private void wrapNonAppData() throws Exception {
		...
		try {
			loop:while(true) {
				...
				if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					log.debug("NonAppWrap result " + result + " buffers size "
	...
	private void startBuffer(ByteBuffer src) {
		if(tlsRecordBuffer == null) {
			...
			if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				log.debug("Allocated record buffer for reading " + tlsRecordBuffer + " for src = " + src);
			}
		}
	}
	...
	private ByteBuffer normalizeTlsRecordBuffer(ByteBuffer src) {
		if(tlsRecordBuffer == null) {
			...
			if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				log.debug("Normalize buffer " + src + " into record buffer "
						+ tlsRecordBuffer);
			}
		...
		synchronized (unwrapLock) {
			...
			loop:while(true) {
				...
				try {
					...
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("An Exception occured while trying to unwrap the message " + e);
					}
					throw e;
				}
				if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					log.debug("Unwrap result " + result + " buffers size "
							+ pendingOutboundBuffers.size() + " src=" + src + " dst=" + dst);
				}

				if(result.getStatus().equals(Status.BUFFER_UNDERFLOW)) {
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Buffer underflow, wait for the next inbound chunk of data to feed the SSL engine");
					}
					startBuffer(src);
					break;
				} else {
				...
				if(result.getStatus().equals(Status.BUFFER_OVERFLOW)) {
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Buffer overflow , must prepare the buffer again."
					...
					dst = newBuf;
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug(" new outNetBuffer remaining: " +  dst.remaining()
				...
				case NEED_UNWRAP:
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Unwrap has remaining: " + src.hasRemaining() + " buffer " + src);
					}
					if(src.hasRemaining()) {
						break;
					} else {
				...
				case FINISHED:
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Handshaking just finnished, but has remaining. Will try to wrap the queues app items.");
					}
					wrapRemaining();
					if(src.hasRemaining()) {
						break;
				...
				case NOT_HANDSHAKING:
					wrapRemaining();
					if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						log.debug("Not handshaking, but has remaining: " + src.hasRemaining() + " buffer " + src);
					}
					if(src.hasRemaining()) {
						break;
					} else {
	...
	private void runDelegatedTasks(SSLEngineResult result) throws IOException {
		if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			log.debug("Running delegated task for " + result);
		}
		...
		if (result.getHandshakeStatus() == HandshakeStatus.NEED_TASK) {
			...
			if(logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				log.debug("Handshake status after delegated tasks " + hsStatus);
			}
