...
public class HandshakeCompletedListenerImpl implements HandshakeCompletedListener {
    ...
    public void startHandshakeWatchdog() {
        if (this.watchdog != null) {
        	log.info("starting watchdog for socket " + watchdog.socket + " on sslhandshake " + sipStack.getSslHandshakeTimeout());
	...
	class HandshakeWatchdog extends SIPStackTimerTask {
		...
		@Override
		public void runTask() {
			log.info("closing socket " + socket + " on sslhandshaketimeout");
			 try {
                 socket.close();
             } catch (IOException ex) {
                 log.info("couldn't close socket on sslhandshaketimeout");
             }
			 log.info("socket closed " + socket + " on sslhandshaketimeout");
		}
