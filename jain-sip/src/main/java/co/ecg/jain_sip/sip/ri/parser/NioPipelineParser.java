    ...
    public class Dispatch implements Runnable, QueuedMessageDispatchBase {
        ...
        public void run() {
            ...
            try {
                ...
                if (!acquired) {
                    log.warn("Semaphore acquisition for callId " + callId + " wasn't successful so don't process message, returning");
                    // https://java.net/jira/browse/JSIP-499 don't process the message if the semaphore wasn't acquired
                    return;
                } else {
                    log.debug("semaphore acquired for message " + callId + " acquired");
                }
            ...
            try {
                synchronized (smp) {
                    ...
                    if (parsedSIPMessage == null) {
                        ...
                        log.debug("parsed message is null, probably because of end of stream, empty packets or socket closed "
                ...
                if (messagesForCallID.size() <= 0) {
                    ...
                    log.debug("CallIDOrderingStructure removed for callId " + callId);

                }

                log.debug("releasing semaphore for message " + parsedSIPMessage);
            ...
            log.debug("dispatch task done on " + parsedSIPMessage);
        ...
        if (partialLineRead) {
            ...
            if (!line.equals(CRLF)) { // CRLF indicates END of message headers by RFC
                ...
                if (message.length() > 0) { // if we havent read any headers yet we are between messages and ignore CRLFs
                    ...
                    log.debug("Content Length parsed is " + contentLength);
                ...
                if (orderingStructure == null) {
                    ...
                    if (orderingStructure == null) {
                        orderingStructure = newCallIDOrderingStructure;
                            ...
                            log.debug("new CallIDOrderingStructure added for message " + message);
