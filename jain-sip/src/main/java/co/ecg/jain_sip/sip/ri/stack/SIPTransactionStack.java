...
//        this.timer = new DefaultTimer();
    ...
    public SIPDialog putDialog(SIPDialog dialog) {
        ...
        log.debug("putDialog dialogId=" + dialogId
                + " dialog = " + dialog);
    ...
    public SIPDialog createDialog(SIPTransaction transaction) {
        ...
        if (transaction instanceof SIPClientTransaction) {
            ...
            log.debug("createDialog dialogId=" + dialogId);
            ...
            if (this.earlyDialogTable.get(dialogId) != null) {
                ...
                if (dialog.getState() == null
                    ...
                    log.debug("createDialog early Dialog found : earlyDialogId="
                ...
                log.debug("createDialog early Dialog not found : earlyDialogId="
    ...
    public SIPDialog createDialog(SIPClientTransaction transaction,
        ...
        log.debug("createDialog originalDialogId=" + originalDialogId);
        log.debug("createDialog earlyDialogId=" + earlyDialogId);
        log.debug("createDialog default Dialog=" + transaction.getDefaultDialog());
        if (transaction.getDefaultDialog() != null) {
            log.debug("createDialog default Dialog Id=" + transaction.getDefaultDialog().getDialogId());
        }
    ...
    public SIPDialog getEarlyDialog(String dialogId) {
        ...
        log.debug("getEarlyDialog(" + dialogId + ") : returning " + sipDialog);
    ...
    protected void removeMergeDialog(String mergeId) {
        if (mergeId != null) {
            log.debug("Tyring to remove Dialog from serverDialogMerge table with Merge Dialog Id " + mergeId);
            SIPDialog sipDialog = serverDialogMergeTestTable.remove(mergeId);
            log.debug("removed Dialog " + sipDialog + " from serverDialogMerge table with Merge Dialog Id " + mergeId);
        }
    }
    ...
    protected void putMergeDialog(SIPDialog sipDialog) {
        if (sipDialog != null) {
            ...
            if (mergeId != null) {
                ...
                log.debug("put Dialog " + sipDialog + " in serverDialogMerge table with Merge Dialog Id " + mergeId);
            }
        }
    }
    ...
    public void removeDialog(String dialogId) {
        ...
        log.warn("Silently removing dialog from table");
    ...
    public SIPClientTransaction findSubscribeTransaction(
        ...
        try {
            ...
            log.debug("findSubscribeTransaction : returning "
                    + retval);
...
//        String branchId = ((SIPRequest)serverTransaction.getRequest()).getTopmostVia().getBranch();
    ...
    public boolean findMergedTransaction(SIPRequest sipRequest) {
        ...
        if (mergeId != null) {
            ...
            if (mergedTransaction != null
                ...
                log.debug("Mathcing merged transaction for merge id " + mergeId + " with " + mergedTransaction);
                ...
                if (serverDialog != null && serverDialog.firstTransactionIsServerTransaction
                    ...
                    log.debug("Matching merged dialog for merge id " + mergeId + " with " + serverDialog);
        ...
        if (sipMessageValve != null) {
            ...
            try {
                if (!sipMessageValve.processRequest(
                    ...
                    log.debug("Request dropped by the SIP message valve. Request = " + requestReceived);

                    return null;
                }
            } catch (Exception e) {

                log.error("An issue happening the valve on request " +
                        requestReceived +
    ...
    public ServerResponseInterface newSIPServerResponse(
        ...
        if (sipMessageValve != null) {
            ...
            try {
                if (!sipMessageValve.processResponse(
                    ...
                    log.debug("Response dropped by the SIP message valve. Response = " + responseReceived);
                    return null;
                }
            } catch (Exception e) {
                log.error("An issue happening the valve on response " +
                        responseReceived +
                        " thus the message will not be processed further", e);
                return null;
            }
        ...
        if (acquired) {
            ...
            log.debug("Could not acquire semaphore !!");
        }
    ...
    public void addTransaction(SIPClientTransaction clientTransaction) {
        log.debug("added transaction " + clientTransaction);
    ...
    public void removeTransaction(SIPTransaction sipTransaction) {
        ...
        log.debug("removeTransaction: Removing Transaction = "
        ...
        try {
            if (sipTransaction instanceof SIPServerTransaction) {
                ...
                log.debug("REMOVED client tx " + removed + " KEY = "
                        + key);
                ...
                if (removed != null) {
                    ...
                    if (forkId != null && clientTx.isInviteTransaction()
                        ...
                        log.debug("Scheduling to remove forked client transaction : forkId = " + forkId
            ...
            log.debug(String.format("removeTransaction: Table size : " +
    ...
    public void addTransaction(SIPServerTransaction serverTransaction)
        ...
        log.debug("added transaction " + serverTransaction);
            ...
            if (!this.unlimitedClientTransactionTableSize) {
                if (this.activeClientTransactionCount.get() > clientTransactionTableHiwaterMark) {
                    try {
                        ...
                        log.error("Exception occurred while waiting for room", ex);
                    }
                }
            } else {
                this.activeClientTransactionCount.incrementAndGet();
    ...
    protected void removeTransactionHash(SIPTransaction sipTransaction) {
        ...
        if (sipTransaction instanceof SIPClientTransaction) {
            String key = sipTransaction.getTransactionId();
            log.debug("removing client Tx : " + key);
            removed = clientTransactionTable.remove(key);
        } else if (sipTransaction instanceof SIPServerTransaction) {
            String key = sipTransaction.getTransactionId();
            removed = serverTransactionTable.remove(key);
            log.debug("removing server Tx : " + key);
        }
        // http://java.net/jira/browse/JSIP-420
        if (removed != null) {
            ((SIPTransaction) removed).cancelMaxTxLifeTimeTimer();
    ...
    public MessageChannel createRawMessageChannel(String sourceIpAddress,
        ...
        while (processorIterator.hasNext() && newChannel == null) {
            ...
            if (nextHop.getTransport().equalsIgnoreCase(
                ...
                try {
                    ...
                    log.info("UnknownHostException", ex);
                    throw ex;
                } catch (IOException e) {
        ...
        if (newChannel == null) {
            if (log.isDebugEnabled()) {
                log.debug("newChannel is null, messageProcessors.size = " + messageProcessors.size());
                processorIterator = messageProcessors.iterator();
                while (processorIterator.hasNext() && newChannel == null) {
                    nextProcessor = (MessageProcessor) processorIterator.next();
                    log.debug("nextProcessor:" + nextProcessor + "| transport = " +
                            nextProcessor.getTransport() + " ipAddress=" +
                            nextProcessor.getIpAddress() + " port=" +
                            nextProcessor.getPort());
                }
                log.debug("More info on newChannel=null");
                log.debug("nextHop=" + nextHop + " sourceIp=" + sourceIpAddress +
    ...
    public Dialog getReplacesDialog(ReplacesHeader replacesHeader) {
        ...
        log.debug("Looking for dialog " + did);
...
//            throw new IllegalStateException("Stack has been stopped, no further tasks can be scheduled.");
    ...
    public void addForkedClientTransaction(
        ...
        log.debug("Adding forked client transaction : " + clientTransaction +
                " branch=" + clientTransaction.getBranch() +
    ...
    public void setThreadPriority(int threadPriority) {
        ...
        log.info("Setting Stack Thread priority to " + threadPriority);
...
//        } else 
        if (reliableConnectionKeepAliveTimeout == 0) {
            log.info("Default value (840000 ms) will be used for reliableConnectionKeepAliveTimeout stack property");
            reliableConnectionKeepAliveTimeout = 840000;
        }

        log.info("value " + reliableConnectionKeepAliveTimeout + " will be used for reliableConnectionKeepAliveTimeout stack property");
    ...
    public boolean setKeepAliveTimeout(String myAddress, int myPort, String transport, String peerAddress,
        ...
        log.debug("~~~ Trying to find MessageChannel and set new KeepAliveTimeout( myAddress=" + myAddress + ", myPort=" + myPort
    ...
    public boolean closeReliableConnection(String myAddress, int myPort, String transport, String peerAddress, int peerPort) {
        ...
        if (processor != null && processor instanceof ConnectionOrientedMessageProcessor) {
            ...
            log.debug("~~~ closeReliableConnection( myAddress=" + myAddress + ", myPort=" + myPort
                    + ", transport=" + transport + ", peerAddress=" + peerAddress + ", peerPort=" + peerPort
