...
public class WebSocketCodec {
	...
	public byte[] decode(InputStream is)
		...
		do {
			...
			if(availToRead > bytesLeft - 1) {
				...
				if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
					log.debug("Increasing buffer size from " + decodeBuffer.length +
		...
		if(writeIndex<4) {
			if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
				log.debug("Abort decode. Write index is at " + writeIndex);
			}
			return null;
		}
		...
		if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
			log.debug("Decoding WebSocket Frame opCode=" + frameOpcode);
		}
		...
		try {
			...
			if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
				log.debug("Decoding WebSocket Frame length=" + framePayloadLength);
			}
		...
		if(writeIndex < totalPacketLength) {
			if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
				log.debug("Abort decode. Write index is at " + writeIndex + " and totalPacketLength is " + totalPacketLength);
			}
		...
		if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
			log.debug("writeIndex = " + writeIndex + " " + totalPacketLength);
		}
	...
	protected static byte[] encode(byte[] msg, int rsv, boolean fin, boolean maskPayload, byte opcode) throws Exception {
		...
		if(logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
			log.debug("Encoding WebSocket Frame opCode=" + opcode + " length=" + length);
		}
